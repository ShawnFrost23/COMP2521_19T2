...
void IntListInsertInOrder (IntList L, int v)
{
	...
	// Always start by adding size.
	L->size++;

	// Create a temporary head node.
	struct IntListNode *head = L->first;
	// Create a temporary curr node.
	struct IntListNode *curr = L->first;
	// Create a temporary prev node.
	...
	if (head == NULL) {
		L->first = new_node;
		L->last = new_node;
	} else if (new_node->data <= head->data) { // Smallest value.
		new_node->next = L->first;
		L->first = new_node;
	} else if (new_node->data >= L->last->data) { // Largest value
		L->last->next = new_node;
		L->last = new_node;
	} else {   // Rest of cases
		//while loop to reach our desired node, where we wish to add our number.
