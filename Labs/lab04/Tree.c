...
static Link insert (Link t, Item it)
{
	...
	int diff = cmp (key (it), key (t->value), t->within);
...
static Link insertSplay (Link t, Item it)
{
	...
	Key v = key (it);
	int diff = cmp (v, key (t->value), t->within);
	if (diff == 0) {
		t->value = it;
		...
		if (t->left == NULL) {
			t->left = newNode (it);
		} else if (less (v, key (t->left->value), t->within)) {
		...
		if (t->right == NULL) {
			t->right = newNode (it);
		} else if (less (key (t->right->value), v, t->within)) {
...
static Link search (Link t, Key k)
{
	if (t == NULL)
		return NULL;
	Link res = NULL;
	int diff = cmp (k, t->value, t->within);
...
static Link searchSplay (Link t, Key k, int *found)
{
	...
	if (t == NULL) {
		...
		*found = 0;
		res = NULL;
	} else if (eq (key (t->value), k, t->within)) {

		*found = 1; // item found, store true
		res = t;
	} else if (less (k, key (t->value), t->within)) {

		if (t->left == NULL) {
			*found = 0; // item not found
			// res = rotateRight(t);
			res = t;
		} else if (eq (key (t->left->value), k, t->within)) {

			*found = 1;
			res = rotateR (t);
		} else {
			if (less (k, key (t->left->value), t->within)) {
		...
		if (t->right == NULL) {
			*found = 0; // item not found
			// res = rotateLeft(t);
			res = t;
		} else if (eq (key (t->right->value), k, t->within)) {

			*found = 1;
			res = rotateL (t);
		} else {
			if (less (key (t->right->value), k, t->within)) {
