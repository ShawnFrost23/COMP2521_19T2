...
{
	assert (L != NULL);
	// Create the new node with our item
	DLListNode *newNode = newDLListNode(it);

	// Increase the number of items
	L->nitems += 1;

	if (L->curr == NULL) { // Null case
		L->curr = L->first = L->last = newNode;
	} else if (L->curr == L->first) { // Case when adding before first node in L
		newNode->next = L->curr;
		L->curr->prev = newNode;
		L->curr = newNode;
		L->first = newNode;
...
void DLListAfter (DLList L, char *it)
{
	...
	if (L->curr == NULL) { // Null case
		L->curr = L->first = L->last = newNode;
	} else if (L->curr == L->last) { // Case when adding after last node in L
		newNode->prev = L->curr;
		L->curr->next = newNode;
		L->curr = L->last = newNode;
	} else { // Rest cases
		newNode->prev = L->curr;
...
void DLListDelete (DLList L)
{
	...
	if (L->curr == NULL) { // Null case
		return;
	} else if (L->first == L->last) { // Case when only one node is there
		tempNode = L->curr;
		freeDLListNode(tempNode);
		L->curr = L->first = L->last = NULL;
	} else if (L->curr == L->first) { // Case when deleting the first node.
		tempNode = L->curr;
		L->curr = L->curr->next;
		L->curr->prev = NULL;
		L->first = L->curr;
		freeDLListNode(tempNode);
	} else if (L->curr == L->last) { // Case when deleting the last node
		tempNode = L->curr;
		L->curr = L->curr->prev;
		L->curr->next = NULL;
		L->last = L->curr;
		...
		tempNode = L->curr;
		L->curr = L->curr->next;
		L->curr->prev = tempNode->prev;
		tempNode->prev->next = L->curr;
		freeDLListNode(tempNode);
	}
